...
import { Container, Header, Input, Dropdown, Grid, Segment, Message, Statistic, Button } from 'semantic-ui-react';
...
const FindQuestionPage = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      console.log('Processed questions data:', data);
      setQuestions(data);
      setLoading(false);
    }, (err) => {
...
const FindQuestionPage = () => {
  ...
  const handleAddSampleData = async () => {
    ...
    try {
      ...
      setError('Failed to add sample data: ' + err.message);
    }
          ...
          {questions.length === 0 && !loading && (
            <Grid.Row>
              <Grid.Column width={16} textAlign="center">
                ...
                >
                  {addingSampleData ? 'Adding Sample Data...' : 'Add Sample Posts'}
                </Button>
                <p style={{ marginTop: '10px', color: '#666' }}>
                  No posts found. Click above to add some sample posts to get started!
                </p>
              </Grid.Column>
            </Grid.Row>
          )}
